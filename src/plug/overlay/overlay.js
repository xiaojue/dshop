 ...
 * @version 20111011
 * @fileoverview 需要以后重写，暂时根据demo的接口来用
 */
(function(W, doc, $) {
	dshop.add('overlay', function() {
		/**
   * @memberOf jQuery
   * @constructor
	 ...
	 */
       ...
       */
          ...
          that.destroy(); //先用destroy清除一下生成的dom，使new出来的对象的dom节点可以重新绘制
					...
					//高度的获取有时候会延迟在IE6下,且不输入width和height的时候，自动获取容器高宽
					setTimeout(function() {
       ...
       */
          ...
          $('#'+config.coverId).css('opacity',opacity);
				},
				/**
       ...
       */
				...
				_cover: function() {
					...
					if (ie6) {
						cover.append("<iframe style='width:100%;" + "filter:alpha(opacity=0);" + "z-index:-1;'>");
      ...
      return _overlay._init;
    }();

    dshop.mods['overlay'] = overlay;
